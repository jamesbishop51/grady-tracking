  ...
  <GdContainer>
    <GdCard class="p-4">
      <div class="fex flex-col gap-4">
        ...
        <select
          class="mt-1 block w-full rounded-md outline outline-offset-1 outline-1 outline-secondary border-gray-300 py-2 pl-3 pr-10 text-base focus:border-secondary focus:outline-none focus:ring-secondary sm:text-sm"
          @change="updateUser"
        >
          <option value="">Select Operator</option>
          <option
            v-for="opt in store.operatorsToOptions"
            :key="opt.value"
            :value="opt.value"
          >
            {{ opt.text }}
        ...
        <select
          class="mt-1 block w-full rounded-md outline outline-offset-1 outline-1 outline-secondary py-2 pl-3 pr-10 text-base focus:border-secondary focus:outline-none focus:ring-secondary sm:text-sm"
          v-model="store.currentUser.task"
        >
          <option value="">Select Task</option>
          <option v-for="options in store.currentUserTasks">
            {{ options.text }}
          </option>
        </select>
